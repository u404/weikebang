  ...
  flipTimer.prototype = {
    ...
    renderDigits: function(subject, value) {
      ...
      if ($(subject).find('.digit').length == 0) {
        ...
        if (value > 99) {
            var digitSetHtml = '';
            for (var i = 0, valLen = String(value).length; i < valLen; i++) {
                digitSetHtml += '<div class="digit-set"></div>';
            }
            $(subject).append(digitSetHtml);
        }
        else {
    ...
    startTimer: function() {
      ...
      this.timer = setInterval(function () {
        ...
        //  return;
          //}
        if ((_this.days == 99 && _this.hours == 23 && _this.minutes == 59 && _this.seconds == 59)) {
            clearInterval(_this.timer);
            return;
        }
        ...
        var dateDiff = _this.options.date - nowDate;

          // sets the date/time on the instance
        if(_this.options.seconds)

        _this.seconds = Math.floor(dateDiff / 1000) % 60;
        var minutes = Math.floor(dateDiff / 1000 / 60) % 60;
        var hours = Math.floor(dateDiff / 1000 / 3600) % 24;
        var days = Math.floor(dateDiff / 1000 / 60 / 60 / 24);

        if (_this.options.seconds) _this.updateDigit(_this.options.seconds,_this.seconds);
        if (_this.options.minutes && minutes != _this.minutes) {
            _this.minutes = minutes;
            _this.updateDigit(_this.options.minutes,_this.minutes);
        }
        if (_this.options.hours && hours != _this.hours) {
            _this.hours = hours;
            _this.updateDigit(_this.options.hours, _this.hours);
        }
        if (_this.options.days && days != _this.days) {
            _this.days = days;
            _this.updateDigit(_this.options.days, _this.days);
        }
    ...
    updateDigit: function (target, num) {
        ...
        var digitCount = digitSets.length;
        var numStr = num + '';
        for (var i = numStr.length; i < digitCount; i++) {
            numStr = '0' + numStr;
        }
        var numArr = numStr.split('');
        for (var i = 0; i < digitCount; i++) {
            updateElem(digitSets[i], numArr[i]);
        }
         ...
         */
        function updateElem(el, digit) {
            var current = $(el).find('.active'),
                previous = $(el).find('.previous'),
                index = $.inArray(el, digitSets);

            var maxDigit = $(el).children().length - 1;
            var digitIndex = maxDigit - digit;
            if (digitIndex == current.index()) {
                return;
            }
            var $new = $(el).children().eq(digitIndex);
    ...
    restart: function (options) {
        this.distroy();
        this.element.data('flipTimer', new flipTimer(this.element, $.extend({}, this.options, options)));
    },
    stop: function () {
        var oldFlipTimer = this.element.data('flipTimer');
        if (oldFlipTimer) {
            clearInterval(oldFlipTimer.timer);
        }
    },
    distroy: function () {
        this.stop();
        this.element.removeData('flipTimer').removeClass('flipTimer').html(this.element.data('source'));
    }
  };
