...
(function ($) {
	...
	var instanceCount = 0;
	$.fn.initMDateTimer = function (opts) {
	    var nowYear = new Date().getFullYear();
		var defaults = {
			mode : 2, //时间选择器模式：1：年月日，2：年月日时分（24小时），3：年月日时分（12小时），4：年月日时分秒
			format : 2, //时间格式化方式：1：2015年06月10日 17时30分46秒，2：2015-05-10  17:30:46
			years: nowYear + '~' + (nowYear + 2), //年份，[2015,2016,2017]数组或字符串'2000~2015'
			nowbtn : true,
			onOk : null,
			onCancel : null
		};
		...
		input.each(function(index, element){
			var $element = $(element);
			var id = ++instanceCount; //$element.attr('id');
			...
			var picker = {
				init : function(){
					...
					var yearStr = '';
					if (typeof option.years === 'string') {
					    var yearSplitArr = option.years.split('~');
					    var yearStart = yearSplitArr[0];
					    var yearEnd = yearSplitArr[yearSplitArr.length - 1];
